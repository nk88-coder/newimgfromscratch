...
# ✅ Final Inference Script for Your Trained VQ-VAE + Transformer Model
# ✅ Matches EXACT architecture from training (incl. LSTM + self/cross blocks)
...
class EMAQuantizer(nn.Module):
    ...
    def __init__(self, num_embeddings=1024, embedding_dim=512, commitment_cost=0.25, decay=0.99, eps=1e-5):
        ...
        self.register_buffer("ema_cluster_size", torch.zeros(num_embeddings))  # ✅ include this
        self.ema_w = nn.Parameter(embed.clone(), requires_grad=False)          # ✅ include this
...
if __name__ == "__main__":
    try:
        ...
        args, _ = parser.parse_known_args()  # ✅ Avoid crash on unknown args
    except:
        class Args: prompt = "a dog standing on the park"
        args = Args()
    ...
    img = generate_image(vqvae, transformer, text_embed)[0].permute(1, 2, 0).cpu().numpy()
    img = (img + 1) / 2
    plt.imshow(img)
    plt.axis("off")
